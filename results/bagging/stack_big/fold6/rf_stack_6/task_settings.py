...
from my_utils import try_convert_to_number
...
class Settings:
    ...
    allowed_test_specifications = [must_be_known, must_not_be_known, atom_test_type_constant]
    ...
    allowed_sections = [sec_relations, sec_aggregates, sec_atom_tests, sec_tree_params,
                        sec_relations_constructed]
    comment_string = "//"
    default_tree_parameters = {"numNodes": float("inf"), "minInstancesNode": 1, "maxDepth": float("inf"),
                               "maxTestLength": 1}
    ...
    def __init__(self, s_file):
        ...
        self.tree_parameters = {k: None for k in Settings.default_tree_parameters}
        ...
        with open(s_file) as f:
            ...
            section_indices = []
            for l in f:
                i = l.find(Settings.comment_string)
                line = l.strip() if i < 0 else l[:i].strip()
                if line:
            ...
            for i in range(len(section_indices) - 1):
                self.parse_section(lines[section_indices[i]:section_indices[i + 1]])
        self.sanity_check()
    ...
    def get_atom_tests_structured(self):
        allowed_specs = {s: i for i, s in enumerate(Settings.allowed_test_specifications)}
        structured = {}
        ...
        for r, specifications in self.atom_tests:
            ...
            for i, (specification, o_type) in enumerate(zip(specifications, rel_data_types[r])):
    ...
    def parse_section(self, lines):
        ...
        if sec_name == Settings.sec_relations:
            for line in data:
                ...
                new_relation = Relation(relation_name, set(), None, relation_types)
                if new_relation not in self.relations:
                    self.relations.append(new_relation)
                else:
                    print("Warning: relation {} was listed more than once. Ignoring duplicates.".format(new_relation))
        elif sec_name == Settings.sec_aggregates:
            for line in data:
                if line not in Settings.allowed_aggregators:
                    raise WrongValueException("Wrong aggregator: {}. Allowed: {}".format(line,
                                                                                         Settings.allowed_aggregators))
                if line not in self.aggregates:
                    self.aggregates.append(line)
                else:
                    print("Warning: aggregate {} was listed more than once. Ignoring duplicates.".format(line))
        elif sec_name == Settings.sec_atom_tests:
            for line in data:
                relation_name = line[:line.find("(")]
                test_specifications = parse_relation_arguments(line, relation_name)
                for t in test_specifications:
                    if t not in Settings.allowed_test_specifications:
                        message = "Wrong test specification: {}. Allowed: {}"
                        raise WrongValueException(message.format(t, Settings.allowed_test_specifications))
                new_test = (relation_name, tuple(test_specifications))
                if new_test not in self.atom_tests:
                    self.atom_tests.append(new_test)
                else:
                    print("Warning: test {} was listed more than once. Ignoring duplicates.".format(new_test))
        elif sec_name == Settings.sec_tree_params:
            for line in data:
                key_value = re.match("([A-Za-z]+) *= *(.+)", line)
                if key_value is None:
                    raise WrongValueException("Wrong line format for {}.\nUse parameter = value format.".format(line))
                ...
                if key not in self.tree_parameters:
                    ...
                    raise WrongValueException(message.format(key, list(self.tree_parameters.keys())))
                if self.tree_parameters[key] is not None:
                    print("Warning! Tha value for {} is already defined. Will be overridden.".format(key))
                self.tree_parameters[key] = value
            for k, v in self.tree_parameters.items():
                if v is None:
                    self.tree_parameters[k] = Settings.default_tree_parameters[k]
            ...
            raise WrongValueException(message.format(sec_name, Settings.allowed_sections))
    ...
    def sanity_check(self):
        ...
        if len(self.relations) == 0:
            raise MissingValueException("Target relation unknown, because no relation specified.")
        else:
            ...
            for i, t in enumerate(target_types):
                is_ok = (i == len(target_types) - 1) == Relation.is_constant_type(t)
                help_string = "" if (i == len(target_types) - 1) else "not "
                if not is_ok:
                    message = "The type on position {}/{} of target relation must {}be in {}, but is {}."
                    raise WrongValueException(message.format(i + 1, len(target_types),
                                                             help_string,
                                                             Relation.relation_type_constant, t))
        if len(self.atom_tests) == 0:
            print("No atom tests specified, all possible options will be considered.")
            ...
            for r2 in self.relations:
                if r == r2.get_name():
                    ...
                    if len(types) != len(r2.get_types()):
                        ...
                        raise WrongValueException(message.format(r, r2.get_types(), types))
            if not found_r:
                raise MissingValueException("Relation name in atom test: {} not among the relations".format(r))
