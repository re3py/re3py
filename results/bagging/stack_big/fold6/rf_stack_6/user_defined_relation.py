...
from variables import ConstantVariable, VariableVariable
from relation import Relation, parse_relation_arguments
from aggregators import Aggregator, CRITICAL_VALUES, ALL_AGGREGATORS
from comparators import ALL_COMPARATORS
from my_exceptions import WrongValueException
from data import compute_all_values_of_types
import itertools
...
class BinarySplitForRelation(BinarySplit):
    ...
    def get_test_value_helper(self,
                              example,
                              chain_relations, chains_aggregators,
        ...
        def results_generator(related_objects):
            for r in related_objects:
                ...
                # compute
                yield self.get_test_value_helper(example,
                                                 chain_relations,
                                                 chains_aggregators,
                                                 nb_fresh_vars,
                                                 fresh_indices,
        ...
        def should_keep_value(value):
            return not(self.ignore_critical_values and value in CRITICAL_VALUES)
        ...
        if updater is not None:
            updater.is_related(tuple([rel_variables[i_known].value for i_known in known]))
        related = relation.get_all(rel_variables, known)
        if len(chain_relations) == depth:
            if nb_fresh_vars is None:
                fresh_indices = [i for i in unknown if rel_variables[i].can_vary()]
                nb_fresh_vars = len(set(rel_variables[i].get_name() for i in fresh_indices))
            if nb_fresh_vars == 0:
                to_aggregate = [len(related)]
            elif nb_fresh_vars == 1:
                to_aggregate = [r[fresh_indices[0]] for r in related]  # flattened
            else:
                to_aggregate = [[r[i] for i in fresh_indices] for r in related]
            return [to_aggregate for _ in range(len(chains_aggregators))]
        else:
            next_aggregators = [chain[depth] for chain in chains_aggregators]  # type: List[Aggregator]
            to_aggregate = []
...
class RelationConstructor:
    def __init__(self, relation, variables, binary_splits_for_relation: List['BinarySplitForRelation']):
            ...
            for atom_test in binary_split.test:
                ...
                for n, t in zip(var_names, relation_types):
                    if n not in self.all_variables:
                        self.all_variables[n] = RelationConstructor.create_variable(n, t)
...
def compute_new_relations(known_relations: Dict[str, Relation],
    ...
    for t in new_relations_descriptions:
        ...
        relation_types = parse_relation_arguments(name_and_types, relation_name)
        Relation.check_has_ok_types(relation_types)
        fresh[relation_name] = Relation(relation_name, set(), None, relation_types)
        variables = [RelationConstructor.create_variable(n, t) for n, t in zip(variable_names, relation_types)]
        fresh_helpers[relation_name] = RelationConstructor(fresh[relation_name], variables, [])
        # some sanity checks
        ...
        for description in options:
            ...
            if comparator_name not in allowed_comparators:
                raise WrongValueException("Wrong comparator name: {}".format(description[1]))
            for atom_test in atom_tests:
                if atom_test[2] not in allowed_aggregators:
                    raise WrongValueException("Wrong aggregator name: {}".format(atom_test[2]))
    all_relations = {}
    ...
    for t in new_relations_descriptions:
        ...
        for description in options:
            ...
            for atom_test in atom_tests:
                ...
                bs_atom_tests.append((r, r_variable_names, allowed_aggregators[aggregator_name]))
                updater = fresh_helpers[r_name] if r_name in fresh else None
                relation_updaters.append(updater)
            # TODO: compute return Type etc.
            bs = BinarySplitForRelation(relation_updaters, bs_atom_tests, bs_comparator, bs_threshold, bs_ignore, True)
            relation_constructor.binary_splits_for_relation.append(bs)
        relation_constructor.update_all_variables()
    # fill with values
    values_per_type = compute_all_values_of_types(list(known_relations.values()))
