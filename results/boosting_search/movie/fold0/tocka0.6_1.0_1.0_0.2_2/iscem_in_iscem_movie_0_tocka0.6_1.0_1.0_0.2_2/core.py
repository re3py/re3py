...
class DecisionTree(PredictiveModel):
    ...
    def java_on(self):
        if not self.is_outer_java:
            # open server
            path = "C:/Users/matej/git/relational_ranking/pylog/speedUp.jar"
    ...
    def should_try_find_a_split(self, current_node: TreeNode, target_data: List[Datum]):
        if current_node.get_depth() >= self.max_depth:
            ...
            return False
        elif current_node.get_stats().variability < DecisionTree.eps * self.root_node.get_stats().variability:
            # print("Variability too low: ", self.heuristic.compute_variability(current_node.get_stats()))
            return False
        elif current_node.get_stats().variability < 10 ** -16:
            # print("Variability too low: ", self.heuristic.compute_variability(current_node.get_stats()))
            return False
        else:
            return True
        ...
        def get_out_type(current_super_type, current_types, a: Union[Aggregator, Project]):
            ...
            if current_super_type == TYPE_TUPLE:
                ...
                if t == TYPE_NUMERIC:
                    ...
                    # nominal projection
                    assert a.is_projection
                    return current_super_type, [current_types[a.component]]
                else:
