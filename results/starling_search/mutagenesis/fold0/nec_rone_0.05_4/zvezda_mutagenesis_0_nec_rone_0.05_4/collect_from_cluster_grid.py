...
def collect_results_for_dataset(data_dir):
    ...
    for fold in range(10):
        ...
        for configuration_dir, configuration in configuration_dirs:
            c = tuple(configuration[configuration.find("kriza") + len("kriza"):].split('_'))
            log_files[c] = -1.0
        ...
        if best[1] < 0.0:
            update_later.append(fold)
    options = [best_configurations[fold] for fold in range(len(best_configurations)) if fold not in update_later]
    for fold in update_later:
        print(f"Option for {fold}. fold for {data_dir} chosen at random from the existing folds.")
        best_configurations[fold] = random.choice(options)
...
def collect_all_results_for_dataset(data_dir):
    ...
    for fold in range(10):
        ...
        configuration_dirs = [(os.path.join(fold_dir, d, "results"), d)
                              for d in os.listdir(fold_dir) if os.path.isdir(os.path.join(fold_dir, d))]
        for configuration_dir, configuration in configuration_dirs:
            c = tuple(configuration[len("nec_rone_"):].split('_'))
            log_files[c] = -1.0
            files_to_collect = ["experiment.log"]
            is_ok, file_path = try_unzip(configuration_dir, files_to_collect, "experiment.tar.gz")
            if not is_ok:
...
def collect_all_results(model_dir):
    ...
    columns = [columns[i] for i in [0, 1, 2, -2, -1]]
    matrix = []
    for dataset in os.listdir(model_dir):
        full_path = os.path.join(model_dir, dataset)
        if os.path.isdir(full_path):
            ...
            for fold, configurations in results.items():
                for configuration, acc in configurations.items():
                    ...
                    shrinkage, depth = configuration
                    # matrix.append([dataset, fold, shrinkage, rows, splits, depth, acc])
                    matrix.append([dataset, fold, shrinkage, depth, acc])
...
if __name__ == "__main__":
    ...
    # collect_all_results("../experiments/starling_search")
    # collect_results("../experiments/onlyX_boosting_search")
    collect_results("../experiments/onlyX_boosting_search")
    pass
