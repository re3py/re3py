...
from tree_node_split import BinarySplit
...
import math
import copy
from tree_node_split import TEST_VALUE_MEMO
# from my_memo import used_comp_memo
import subprocess
from communicate_with_java import send_data, send_variables, compute_test_values
...
class TreeNode:
    ...
    def __init__(self,
                 ...
                 stats: Union[NodeStatistics, None],
                 depth: int):
...
class DecisionTree(PredictiveModel):
    ...
    def __init__(self,
                 ...
                 max_relative_number_of_evaluated_tests_per_node: Union[float, str] = 1.0,
        ...
        self.allowed_aggregators = set() if allowed_aggregators is None else set(allowed_aggregators)
    ...
    def print_times(self):
        times = [self.induce_tree_time, self.statistics_time, self.get_test_value_time, self.split_eval_time,
                 self.set_example_values_time, self.find_values_time,
                 self.nominal_tests, self.nominal_tests_time, self.numeric_tests, self.numeric_tests_time]
        names = ["induce", "statistics", "test value", "eval split", "example values", "find values",
                 "nominal tests", "nom. t. time", "numeric tests", "num t. time"]
    ...
    def __str__(self):
        def helper(node: TreeNode):
            ...
            if node.is_leaf():
                ...
                parts = ["{}IF {}:".format(spaces, node.split.__str__(self.all_variables))]
    ...
    def get_absolute_number_tests_from_relative(self, nb_tests):
        ...
        if isinstance(n_rel, str):
            ...
            raise ValueError("Wrong relative number of tests: {}.".format(n_rel))

    def relative_number_tests_sanity_check(self):
        if isinstance(self.max_relative_number_of_evaluated_tests_per_node, str):
            if self.max_relative_number_of_evaluated_tests_per_node not in DecisionTree.allowed_n_tests:
                ...
                raise ValueError(
                    message.format(self.max_relative_number_of_evaluated_tests_per_node, DecisionTree.allowed_n_tests)
                )
        elif isinstance(self.max_relative_number_of_evaluated_tests_per_node, float):
            if not (0.0 <= self.max_relative_number_of_evaluated_tests_per_node <= 1):
                message = "Wrong relative number of tests: {}. If float, should be in the interval (0, 1]"
                raise ValueError(message.format(self.max_relative_number_of_evaluated_tests_per_node))
        else:
            raise ValueError("Relative number of tests should be string or float.")
    ...
    def chosen_tests(self, tests_generator, current_vars_per_type):
        ...
        for jj, chain in enumerate(tests_generator):
            ...
            _, _, _, _, c_num = self.create_example_and_chains(relation_chain, current_vars_per_type)
        ...
        k = min(k_proportion, k_absolute)  # Select the more restrictive criterion
    ...
    def generate_temp_var_names(self, first_letter, offsets):
        names = ["{}{}".format(first_letter, self.temp_var_count - offset) for offset in offsets]
        if names:
    ...
    def initialize_statistics(self, node: TreeNode, data: List[Datum]):
        ...
        initial_stats = statistics_class.construct_from_parent(self.target_data_stat)
    ...
    def build(self, data: Dataset):
        ...
        self.descriptive_data = data.get_descriptive_data()  # type: Dict[str, Relation]
        ...
        for object_type in target_relation.get_types()[:-1]:
            var_name = self.generate_next_var_name(DecisionTree.get_first_var_letter(object_type, True))
        ...
        self.target_relation_description = "{}, {}".format(target_relation.get_name(), target_var_names)
        all_variable_names = set(target_var_names)
        self.root_node = TreeNode(DecisionTree.root_indicator,
                                  None,
                                  [],
                                  None,
                                  None,
                                  DecisionTree.root_node_depth)
        # initial statistics
        ...
        self.build_helper(target_data, self.root_node, current_vars_per_type, target_var_names, all_variable_names)
    ...
    def java_on(self):
        if not self.is_outer_java:
            # open server
            path = "E:/relational_ranking/pylog/speedUp.jar"
            if not os.path.exists(path):
                path = "speedUp.jar"
            self.p = subprocess.Popen(r"java -jar {} {}".format(path, self.java_port), shell=True)
    ...
    def update_allowed_aggregates(self):
        if self.only_existential:
            ...
            print("Existential tests only ==> The allowed aggregates changed to {}".format(self.allowed_aggregators))

    def build_helper(self, target_data: List[Datum], current_node: TreeNode,
                     current_vars_per_type, target_relation_vars, all_variable_names: Set[str]):
        ...
        print("{}Building node on depth {}".format("  " * current_node.get_depth(), current_node.get_depth()))
        current_var_names = [{t: {v.get_name() for v in vs} for t, vs in d.items()} for d in current_vars_per_type]
        ...
        if self.should_try_find_a_split(current_node, target_data):
            ...
            attributes_counting = self.generate_possible_attributes(copy.deepcopy(current_var_names),
                                                                    parents_test,
                                                                    target_relation_vars)
            self.reset_temp_var_count()
            attributes = self.generate_possible_attributes(current_var_names,
                                                           parents_test,
                                                           target_relation_vars)
            ...
            attributes = iter([])
        chosen_attributes, all_attributes_computed, all_chains_computed = self.chosen_tests(attributes_counting,
                                                                                            current_vars_per_type)
        ...
        for chain in attributes:
            ...
            # print(relation_chain)
            example, rc_modified, c_values, c_var_names, c_num = self.create_example_and_chains(relation_chain,
                                                                                                current_vars_per_type)
            ...
            for c_vs in c_values:
                ...
                if BinarySplit.use_memo or known_unknown is None:
                    ...
                    r_key, a_keys, known_unknown = DecisionTree.test_values_memo_keys(example,
                                                                                      rc_modified,
                                                                                      filtered_agg_chains)
                else:
                    r_key, a_keys = None, []
                ...
                if self.java_port is not None:
                    ...
                    all_test_values = compute_test_values(target_data, target_relation_vars, rc_modified,
                                                          filtered_agg_chains,
                                                          r_key, a_keys, nb_fresh_vars, fresh_indices, known_unknown,
                                                          self.client, self.wrapper)
                else:
                    all_test_values = []
                    for jj, datum in enumerate(target_data):
                        if jj == 0 and "KPoTixdjoJxSqRSEApSAGg" == datum.get_descriptive()[0]:
                            _ = 21

                        u0 = clock()
                        for init_var_name, train_value in zip(target_relation_vars, datum.get_descriptive()):
                            example[init_var_name].set_value(train_value)
                        u1 = clock()
                        self.set_example_values_time += u1 - u0
                        u0 = clock()
                        all_test_values.append(bs.get_test_values(example, rc_modified, filtered_agg_chains,
                                                                  r_key, a_keys, datum.identifier,
                ...
                t0 = clock()
                score, configuration = self.evaluate_candidate_splits(current_node,
                                                                      all_test_values,
                                                                      target_data,
                                                                      current_vars_per_type[0],
                                                                      filtered_output_types)
                t1 = clock()
                ...
                if BinarySplit.is_better_than_previous(score, best_score):
                    ...
                    best_configuration = (rc_modified, filtered_agg_chains[a_chain_ind],
                                          c_vs, c_var_names,
                                          comparator, theta,
                                          partition,
                                          is_variable_free,
        ...
        if all_attributes_computed != all_attributes_counted or all_chains_computed != all_chains_counted:
            ...
            raise WrongValueException(message.format(all_attributes_computed, all_attributes_counted,
                                                     all_chains_computed, all_chains_counted))
        # create internal node or leaf
        if BinarySplit.is_better_than_previous(best_score, BinarySplit.worst_split_score):
            ...
            partition, \
            is_variable_free, \
            starting_index = best_configuration
            ...
            for i_relation, ((r, var_names), a) in enumerate(zip(r_chain, a_chain)):
                new_names = []
                for var_name, var_type in zip(var_names, self.descriptive_data[r.get_name()].get_types()):
                    ...
                    if condition1:
                        ...
                        fresh_variables_chain = fresh_variables_chains[i_relation - (starting_index - 1)]
                        # not seen in this chain also?
                        if var_name not in temp_to_fix_name:
                            # we see this name for the first time --> rename from temp name
                            new_var_name = self.generate_next_var_name(var_name[0])
                            ...
                            value = const_name_to_value[var_name] if is_const2 else None
                            new_var = create_new_variable(new_var_name, var_type, value)
            ...
            self.reset_temp_var_count()
            # set the split
            bs = BinarySplit(split_test, comparator, theta, True, is_variable_free)
            bs.add_fresh_variables(fresh_variables)
            current_node.set_split(bs)
            # branch frequencies
            nb_examples = target_data_weight(target_data)
            branch_frequencies = [target_data_weight(target_data, part) / nb_examples for part in partition]
            current_node.get_stats().set_branch_frequencies(branch_frequencies)
            # create children etc.
            current_variables_children = [current_vars_per_type[:starting_index] + fresh_variables_chains,
                                          current_vars_per_type]
            ...
            for i, part in enumerate(partition):
                ...
                child = TreeNode(label, current_node, [], None, None, current_node.get_depth() + 1)
                ...
                self.build_helper(target_data_child,
                                  child,
    ...
    def should_try_find_a_split(self, current_node: TreeNode, target_data: List[Datum]):
        if current_node.get_depth() >= self.max_depth:
            return False
        elif self.current_number_internal_nodes >= self.max_number_internal_nodes:
            return False
        elif target_data_weight(target_data) <= 2 * self.minimal_examples_in_leaf - DecisionTree.eps:
            return False
        elif current_node.get_stats().variability < self.minimal_impurity * self.root_node.get_stats().variability:
            # print("Variability too low: ", self.heuristic.compute_variability(current_node.get_stats()))
            return False
        else:
            return True

    def evaluate_candidate_splits(self, parent, test_values, target_data, target_relation_vars, filtered_output_types):
        ...
        for i in range(n_aggregators):
            ...
            if filtered_output_types[i] == TYPE_NUMERIC:  # is numeric
                t0 = clock()
                score, comparator, theta, partition = self.find_best_numeric(parent, xs, target_data)
                ...
                if output_type in target_relation_vars:
                    var_candidates = sorted([v.name for v in target_relation_vars[output_type]])
                else:
                    var_candidates = []
                score, comparator, theta, partition, is_variable_free = self.find_best_nominal(parent, xs, target_data,
                                                                                               var_candidates,
                                                                                               output_type)
                t1 = clock()
            ...
            if BinarySplit.is_better_than_previous(score, best_score):
                best_score = score
                best_configuration = (i, comparator, theta, partition, is_variable_free)
    ...
    def find_best_nominal(self, parent: TreeNode, xs: List[str], target_data: List[Datum],
        ...
        if self.only_existential:
            raise ValueError("Only existential tests cannot leave to nominal tests.")
        is_usual_nominal = Relation.is_nominal_type(output_type)
        if is_usual_nominal:
            different_values = sorted(set(xs))  # better than list, so that the order is always the same
            ...
            target_relation_var_indices = [int(v[1:]) for v in target_relation_vars]
        n = len(different_values)
        m = min(n, max_set_size)
        if is_usual_nominal:
            options = 2 ** (m - 1)  # Half of them suffice, empty set skipped later if necessary
        else:
            # Possible values are:
        ...
        if n > max_set_size:
            ...
            subsets = my_utils.subsets_of_list(different_values, options)
            next(subsets)  # skip the empty set
        best_score = BinarySplit.worst_split_score
        best_comparator = None
        best_subset = None
        best_partition = None
        generic_statistic = self.target_data_stat.__class__.construct_from_parent(parent.get_stats())
        for left_list, _ in subsets:
            self.nominal_tests += 1
            if is_usual_nominal:
                left = set(left_list)
            partition = [[], []]  # type: List[List[int]]
            statistics = [generic_statistic.get_copy() for _ in range(len(partition))]
            for datum_in, (test_value, datum) in enumerate(zip(xs, target_data)):
                j = 1
                if is_usual_nominal:
                    ...
                    for chosen in left_list:
                        if datum.descriptive_part[target_relation_var_indices[chosen]] == test_value:
                            j = 0
                            break
            ...
            if self.is_split_valid(statistics):
                score = self.heuristic.evaluate_split(parent.get_stats(), statistics)
            else:
                score = BinarySplit.worst_split_score
            if BinarySplit.is_better_than_previous(score, best_score):
                best_score = score
                best_partition = partition
                transpose_branches = DecisionTree.should_transpose_branches(statistics)
                ...
                if is_usual_nominal:
                    ...
                    best_subset = {different_values_helper[chosen] for chosen in left_list}
    ...
    def find_best_numeric(self, parent: TreeNode, xs: List[float], target_data: List[Datum]) \
        ...
        statistics = [generic_statistic,
                      parent.get_stats().get_copy()]  # type: List[NodeStatistics]
            ...
            if x > previous_value:
                if x < float('inf'):
                    if previous_value > float('-inf'):
                        threshold = previous_value + (x - previous_value) / 2  # element of (previous, x)
                    else:
                ...
                if BinarySplit.is_better_than_previous(score, best_score):
                    ...
                    best_i = i
                    transpose_branches = DecisionTree.should_transpose_branches(statistics)
            statistics[TreeNode.negative_branch].remove_example_during_split_eval(datum)
            statistics[TreeNode.positive_branch].add_example_during_split_eval(datum)
        if BinarySplit.is_better_than_previous(best_score, BinarySplit.worst_split_score):
    ...
    def is_split_valid(self, stats: List[NodeStatistics]):
        for stat in stats:
            if stat.get_total_number_examples() <= self.minimal_examples_in_leaf - DecisionTree.eps:
                return False
        return True
    ...
    def generate_possible_attributes(self,
                                     current_var_names,
                                     current_atom_tests,
        ...
        if len(current_var_names) - 1 != len(current_atom_tests):
            raise WrongValueException("Lengths do not differ by one::\n{}\n{}".format(current_var_names,
                                                                                      current_atom_tests))
        var_names_up_to_here = {}
        current_atom_tests_modified = []

        for r, vs, a in current_atom_tests:
            current_atom_tests_modified.append((r.get_name(), list(zip(vs, r.get_types()))))
        for start_index in range(1, 1 + len(current_var_names)):
            var_names_up_to_here = my_utils.union_of_two_dicts(var_names_up_to_here,
                                                               current_var_names[start_index - 1])
            test_chain = current_atom_tests_modified[:start_index - 1]
            for new_tests_chain in self.generate_possible_attributes_helper_all_steps(1,
                                                                                      var_names_up_to_here):
                created_chain = test_chain + new_tests_chain
                if is_relation_chain_valid(created_chain):
                    fresh_v, fresh_i = DecisionTree.fresh_in_last_relation(created_chain, target_relation_vars)
                    aggregator_chains = self.generate_possible_aggregator_chains(len(created_chain),
                                                                                 fresh_v,
                                                                                 fresh_i)
                    yield start_index, created_chain, aggregator_chains, (set(fresh_v), fresh_i)
    ...
    def generate_possible_aggregator_chains(self, tests_chain_len, last_fresh_variables, last_fresh_indices):
        def last_step_helper(fresh: int, current_type: str):
            if fresh == 0:
                ...
                if current_type == TYPE_TUPLE:
                    ...
                    raise WrongValueException("Unknown type: {}".format(current_type))
            chosen = []
            if is_complex and PROJECT.name in self.allowed_aggregators:
                for i, var_index in enumerate(last_fresh_indices):
                    chosen += [PROJECTIONS[var_index](o) for o in last_step_helper(1, last_super_types[i])
                               if o.name != COUNT.name]
            chosen += [a for a in options if a.name in self.allowed_aggregators and a.name != PROJECT.name]
            yield from chosen

        def generator_chains_helper(current_options_gen, current_super_type, current_types, position):
            ...
            for o in current_options:
                next_super_type, next_types = get_out_type(current_super_type, current_types, o)
                if position == 0:
                    ...
                    for left_part, left_type in generator_chains_helper(next_options,
                                                                        next_super_type,
                                                                        next_types,
                                                                        position - 1):
        ...
        def get_out_type(current_super_type, current_types, a: Union[Aggregator, Project]):
            t = a.output_type
            if current_super_type == TYPE_TUPLE:
                # counting or projecting
                if t == TYPE_NUMERIC:
                    # counting, numeric projections
                    return TYPE_NUMERIC, [TYPE_NUMERIC]  # new type can be also simply numeric
                elif t == TYPE_SAME_AS_INPUT:
                    # nominal projection
                    if not a.is_projection:
                        raise ValueError("Should be projection")
                    new_type = current_types[absolute_to_relative_positions[a.component]]
                    new_super_type = get_super_type(1, new_type)
                    return new_super_type, [new_type]
                else:
                    raise WrongValueException("Wrong output type: {}".format(t))
            else:
        ...
        groups = [[a for a in g if a.get_name() in self.allowed_aggregators] for g in [group0, group1, group2, group3]]
        forbidden_first = {FLATTEN_UNIQUE, FLATTEN}
        base_followed_dict = {0: groups[0],
                              1: groups[0],
        ...
        is_followed_by = {TYPE_NUMERIC: is_followed_by_numeric,
        ...
        last_super_types = [get_super_type(1, value_type) for _, value_type in last_fresh_variables]
        ...
        force_use_sum = [nb_fresh == 0]  # to prevent initialisation in helping functions
        absolute_to_relative_positions = {a: r for r, a in enumerate(last_fresh_indices)}
        yield from generator_chains_helper(last_step_helper(nb_fresh, last_super_type),
                                           last_super_type,
                                           last_types,
                                           tests_chain_len - 1)

    def generate_possible_attributes_helper_all_steps(self, depth, var_counts_up_to_here: Dict[str, Set[str]]):
        for head in self.generate_possible_attributes_helper_one_step(var_counts_up_to_here):
            yield [head]
            if depth < self.max_number_atom_tests:
                ...
                new_counts = my_utils.union_of_two_dicts(var_counts_up_to_here, fresh_dict)
                for tail in self.generate_possible_attributes_helper_all_steps(depth + 1, new_counts):
                    yield [head] + tail

    def generate_possible_attributes_helper_one_step(self, var_counts_up_to_here):
        for rel_spec in sorted(self.allowed_atom_tests):
            ...
            for needed_keys, configuration in generate_variable_configurations(var_counts_up_to_here,
                                                                               counts,
                                                                               o_types):
                ...
                for type_config, o_type in zip(configuration, o_types):
                    (ind_old, ind_new), (names_old, relative_counts_new) = type_config
                    # place old
                    ...
                    # place new
                    temp_new_var_names = self.generate_temp_var_names("Y", relative_counts_new)
                    ...
                    for i_const in counts[o_type][2]:  # TODO: hard-coded 2 ...
                        ...
                        none_counter -= 1
                        variable_names[i_const] = self.generate_temp_var_name("C")
    ...
    def create_example_and_chains(self, relations, current_vars):
        ...
        def num_const_values():
            ...
            for c_name in constant_var_names:
                r_name, position = constants[c_name]
                product *= self.descriptive_data[r_name].get_nb_all_values(position)
            return product
            ...
            for i, (n, t) in enumerate(var_names):
                if n not in example:
                    if n[0] == "C":
                        ...
                        raise WrongValueException("Wrong variable name {}".format(n))
            relation_chain.append((self.descriptive_data[relation_name], [n for n, _ in var_names]))

        constant_var_names = sorted(constants.keys())
        c_values = itertools.product(*[const_value_generator(c) for c in constant_var_names])
    ...
    def predict(self, d: Datum, is_for_ensemble=False):
        ...
        for var, value in zip(self.target_relation_variables, d.get_descriptive()):
            example[var.get_name()] = var
            var.set_value(value)
        current_node = self.root_node  # type: TreeNode
        while not current_node.is_leaf():
            # extend the example
            for name, var in current_node.get_split().get_fresh_variables().items():
                example[name] = var
    ...
    def reverse_target_data_induction_preparation(self, target_data: List[Datum]):
        if self.class_weights is None:
            return None
        for d in target_data:
            d.set_weight(d.get_weight() / self.class_weights[d.get_target()])
    ...
    def compute_ranking(self, ranking_type):
        feature_ranking = EnsembleRanking({}, {}, ranking_type, 1)
        attribute_scores, aggregate_scores = feature_ranking.compute_tree_contribution(self)
        feature_ranking.update_attributes(attribute_scores, aggregate_scores, 0)
    ...
    def test_values_memo_keys(example: Dict[str, Variable],
        ...
        for r_vs in relation_chain:
            ...
            for i, v in enumerate(vs):
                u = example[v].is_unset()  # target variables should not be set here yet
                known_unknown.append(not u)
                if u:
                    if v[0] == 'X':
                        out_values.append(v)
                        known_unknown_separated_part[0].append(i)  # but are considered known
                    else:
                        ...
                        if value <= max_overall:
                            known_unknown_separated_part[0].append(i)  # seen in the previous parts
                        else:
                            known_unknown_separated_part[1].append(i)  # unknown until this part
        ...
        aggregators_as_keys = [tuple(a.name for a in a_s) for a_s in aggregation_chains]
...
def generate_variable_configurations(present, needed, needed_keys):
    def generate_config_one_type(p, n):
        old, new, _ = n
        for additional_old, brand_new in my_utils.subsets_of_list(new):
            true_old = old + additional_old
            true_new = brand_new  # new + brand_new?
            old_configs = my_utils.generalized_counting(sorted(p), len(true_old))  # [['X1', 'X2', 'X1'], ...]
            new_configs = my_utils.canonic_sequences_of_new_variables(len(true_new))  # [[1, 3, 2, 1], ...]
    ...
    for combined in itertools.product(*[generate_config_one_type(p, n) for p, n in pairs]):
        yield needed_keys, combined


def random_subsets(l, nb_subsets, random_seed=None):
