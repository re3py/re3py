...
class MeanSquaredError(RegressionEvaluator):
    ...
    def add_one(self, true_value, prediction, weight=1.0):
        super().add_one(true_value, prediction, weight)
        self.sum_of_squared_differences += weight * (true_value - prediction) ** 2
...
class RelativeMeanAbsoluteError(MeanAbsoluteError):
    ...
    def evaluate(self):
        ...
        normalization = sum(w * abs(t - average) for w, t in zip(self.weights, self.true_values)) / self.examples
        self.measure_value /= normalization
...
class ClassificationEvaluator(Evaluator):
    def __init__(self, class_values, positive_class=None, confusion_matrix=None):
        """
        ...
        self.confusion_matrix = [[0 for _ in range(c)] for _ in range(c)]  # matrix[true class][predicted class]
        if confusion_matrix is not None:
            ...
            for i in range(c):
                ...
                for j in range(c):
                    ...
                    n = round(confusion_matrix[i][j])  # round due to fake matrices with 1.0
    ...
    def __str__(self):
        value = str(self.measure_value) if self.measure_value is not None else "None"
    ...
    def __add__(self, other):
        ...
        :return:
        """
        if sorted(self.class_indices.keys()) != sorted(other.class_indices.keys()):
        ...
        if self.positive_class != other.positive_class:
            ...
            exit(-2)
        m = self.__class__(sorted(self.class_indices.keys()), self.positive_class)
        # add matrices
        for term in [self, other]:
            for true_c in term.class_indices:
                ...
                for predicted_c in term.class_indices:
                    ...
                    m.confusion_matrix[i_true_m][i_predicted_m] += term.confusion_matrix[i_true_o][i_predicted_o]
        # add examples
...
class F1(ClassificationEvaluator):
    ...
    def evaluate(self):
        ...
        predicted_positives = sum(x[i_tp] for x in self.confusion_matrix)
        positives = sum(self.confusion_matrix[i_tp])
        self.measure_value = 2 * true_positives / (predicted_positives + positives)
