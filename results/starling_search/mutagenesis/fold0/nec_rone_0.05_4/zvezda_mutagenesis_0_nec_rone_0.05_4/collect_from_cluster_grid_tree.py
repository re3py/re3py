...
def collect_results_for_dataset2(data_dir):
    point_pattern = "tocka(.+)_([0-9]+)"
    best_configurations = []
    for fold in range(10):
        log_files = {}
        fold_dir = os.path.join(data_dir, f"fold{fold}")
        for configuration in os.listdir(fold_dir):
            if re.search(point_pattern, configuration) is None:
                continue
            results_dir = os.path.join(fold_dir, configuration, "results")
...
def collect_results(model_dir, datasets=None, collector=collect_results_for_dataset):
    best_configurations = {}
    for dataset in os.listdir(model_dir):
        ...
        if os.path.isdir(full_path):
            best_configurations[dataset] = collector(full_path)
...
if __name__ == "__main__":
    ...
    collect_results("../experiments/single_tree_search", datasets=["webkb"],
                    collector=collect_results_for_dataset2)
    pass
