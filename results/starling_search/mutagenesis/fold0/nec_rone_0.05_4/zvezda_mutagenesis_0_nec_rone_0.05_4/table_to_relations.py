...
def arff_to_relations(arff_file, target_index, out_dir, key_index=None):
    ...
    if out_dir[-1] != "/":
        out_dir += "/"
    descriptive_relations_file = out_dir + append_to_file_name(arff_name, "_descriptive")
    ...
    for l in f:
        ...
        if l.lower().startswith(attribute):
            ...
            match = re.search(pattern, l, flags=re.I)
    ...
    for l in f:
        ...
        if line:
            ...
            example_id = "ex{}".format(examples) if key_index is None else values[key_index]
            for i, (v, (a_name, a_type)) in enumerate(zip(values, attributes)):
                if i != key_index and v != "?":
                    ...
                    print("{}({}, {})".format(a_name, example_id, v), file=out_f)
