...
class CountUnique(Aggregator):
    def __init__(self):
        ...
        self.input_types = {TYPE_NUMERIC, TYPE_NOMINAL, TYPE_TYPE, TYPE_TUPLE}  # Should numeric be removed?
        self.output_type = TYPE_NUMERIC
...
class Mean(Aggregator):
    ...
    def aggregate_flat(self, l):
        return st.mean(l) if l else float("inf")  # slightly better than 0? None?
    ...
    def aggregate_flat(self, l):
        ...
        for j in l:
            if j not in d:
                ...
                d[j] += 1
        if d:
            return max(sorted(d), key=lambda t: d[t])  # Every time the same element
        else:
...
class Project(Aggregator):
    ...
    def aggregate_flat(self, l):
        return self.aggregator.aggregate_flat([e[self.component] for e in l])
...
PROJECTIONS = [Project0, Project1, Project2, Project3, Project4, Project5, Project6, Project7]

ALL_AGGREGATORS = [FLATTEN, FLATTEN_UNIQUE,
                   COUNT, COUNT_UNIQUE,
                   MIN, MAX, MEAN, SUM,
                   MODE,
                   PROJECT]  # type: List['Aggregator']
NUMERIC_AGGREGATORS = [a for a in ALL_AGGREGATORS
                       if TYPE_NUMERIC in a.input_types]  # type: List['Aggregator']
NOMINAL_AGGREGATORS = [a for a in ALL_AGGREGATORS
                       if TYPE_NOMINAL in a.input_types and TYPE_TYPE in a.input_types]  # type: List['Aggregator']
COMPLEX_ENOUGH_AGGREGATORS = [a for a in ALL_AGGREGATORS
                              if TYPE_TUPLE in a.input_types]
