from core import DecisionTree, create_constant_tree
from my_utils import EnsembleRandomGenerator, average_of_dictionaries
...
from data import get_all_target_values
        ...
        if factor != 1.0:
            for node in tree:
                if node.is_leaf():
                    ...
                    node.get_stats().set_prediction(current_prediction * factor)
...
class GradientBoostingBinaryClassification(GradientBoostingTask):
    ...
    @staticmethod
    def minus_partial_derivative(true_values, predictions):
        return GradientBoostingRegression.minus_partial_derivative(true_values, predictions)
...
class GradientBoostingMulticlassClassification(GradientBoostingTask):
    ...
    def minus_partial_derivative_friedman(true_values, predictions):
        ...
        predictions_exp = [[exp(p) for p in ps] for ps in predictions]  # TODO: numpy? :)
        normalizers = [sum(predictions_exp[k][i] for k in range(classes)) for i in range(examples)]
        prob = [[predictions_exp[k][i] / normalizers[i] for i in range(examples)] for k in range(classes)]
        return [[true_values[k][i] - prob[k][i] for i in range(examples)] for k in range(classes)]

    @staticmethod
    def minus_partial_derivative(true_values, predictions):
        return [GradientBoostingRegression.minus_partial_derivative(t, p) for t, p in zip(true_values, predictions)]
...
class GradientBoosting(TreeEnsemble):
    ...
    def find_task(target_data: List[Datum]):
        ...
        if data_type == float or data_type == int:
            ...
            if c == 2:
                ...
                raise WrongValueException("Weird number of classes: {}".format(c))
        else:
            raise WrongValueException("Wrong target type: {}".format(data_type))

    def build(self, input_data: Dataset):
        # find task
        self.task = GradientBoosting.find_task(input_data.get_target_data())
        if self.task in [GradientBoosting.binary_classification, GradientBoosting.regression]:
    ...
    def build_helper1(self, input_data: Dataset):
        ...
        current_predictions = self.trees[-1].predict_all(data.get_target_data())
        # build boosted trees
        for t in range(self.nb_trees):
            ys = self.task.minus_partial_derivative(true_values, current_predictions)
            modified_data = self.modify_dataset(data, ys)
            print("Building tree {}".format(t + 1))
            self.tree_parameters['random_seed'] = self.ensemble_random.next_tree_seed()
            ...
            self.task.modify_tree(self.shrinkage, self.step_sizes[t], self.optimize_step_size, self.trees[-1])
            GradientBoosting.update_current_predictions(self.trees[-1], current_predictions, data.get_target_data())

    def build_helper2(self, input_data: Dataset):
        # preprocess data
        datasets, class_dictionary = self.preprocess(input_data)
        k = len(datasets)
        true_values = [[datum.get_target() for datum in data.get_target_data()] for data in datasets]
        self.class_dictionary = class_dictionary
        # build default model, for each class
        self.trees_per_class.append([])
        current_predictions = []
        for i in range(k):
            self.trees_per_class[-1].append(self.task.create_default_model(datasets[i]))
            current_predictions.append(self.trees_per_class[-1][i].predict_all(datasets[i].get_target_data()))
        ...
        for t in range(self.nb_trees):
            ...
            self.trees_per_class.append([])
            for i in range(k):
                modified_datasets.append(self.modify_dataset(datasets[i], ys[i]))
                print("Building tree {} for class {}".format(t + 1, i + 1))
                self.tree_parameters['random_seed'] = self.ensemble_random.next_tree_seed()
                self.tree_parameters['heuristic'] = HeuristicVariance()
                self.trees_per_class[-1].append(DecisionTree(**self.tree_parameters))
                self.trees_per_class[-1][i].build(modified_datasets[i])
                self.task.modify_tree(self.shrinkage,
                                      self.step_sizes[t],
                                      self.optimize_step_size,
                                      self.trees_per_class[-1][i])
                GradientBoosting.update_current_predictions(self.trees_per_class[-1][i],
                                                            current_predictions[i],
                                                            datasets[i].get_target_data())

    @staticmethod
    def update_current_predictions(tree, current_predictions, data: List[Datum]):
        ...
        if self.task.is_binary_classification():
            ...
            for target_tuple in data:
                ...
                if t not in dictionary:
                    value = len(dictionary) * 2 - 1 if GradientBoosting.friedman else len(dictionary)
                    dictionary[t] = value
            assert len(dictionary) == 2
            new_target_values = []
            for datum in data:
                new_target_values.append(Datum(datum.get_descriptive(),
                                               dictionary[datum.get_target()],
                                               datum.get_weight(),
                                               datum.identifier))
            ...
            return Dataset(settings=data.settings,
                           ...
                           statistics=new_statistics), {y: x for x, y in dictionary.items()}
            ...
            k = len(dictionary)
            # assert k > 2
            new_target_values = [[] for _ in range(k)]  # type: List[List[Datum]]
            for datum in data:
                for i in range(k):
                    new_target_values[i].append(Datum(datum.get_descriptive(),
                                                      0.0,
                                                      datum.get_weight(),
                                                      datum.identifier))
            if GradientBoosting.friedman:
                new_statistics = NodeStatisticsMulticlassClassificationBoosting(k)
            else:
                new_statistics = NodeStatisticsRegressionBoosting()
            dataset_params = {'settings': data.settings,
                              'descriptive_relations': data.get_descriptive_data(),
    ...
    def modify_dataset(self, data: Dataset, ys):
        ...
        for datum, y in zip(data.get_target_data(), ys):
            new_datum = Datum(datum.get_descriptive(), y, datum.get_weight(), datum.identifier)
    ...
    def compute_ranking(self, ranking_type):
        ...
        for i in range(self.nb_trees):
            if self.task in [GradientBoosting.binary_classification, GradientBoosting.regression]:
                ...
                raise WrongValueException("Unknown task: {}.".format(self.task))
            scores = [feature_ranking.compute_tree_contribution(tree) for tree in trees]
            average_scores = []
            for j in range(2):
                average_scores.append(average_of_dictionaries([pair[j] for pair in scores]))
            feature_ranking.update_attributes(average_scores[0], average_scores[1], i)
    ...
    def predict_helper1(self, d: Datum, nb_trees):
        ...
        if self.task == GradientBoosting.binary_classification:
            ...
            original_classes = [self.class_dictionary[n] for n in numeric_classes]
            if GradientBoosting.friedman:
                p_numeric_classes = []
                for n in numeric_classes:
                    try:
                        p_numeric_classes.append(1 / (1 + exp(-2 * n * prediction)))
                    except OverflowError:
