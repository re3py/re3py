...
from my_utils import EnsembleRandomGenerator
from data import *
...
class RandomForest(TreeEnsemble):
    ...
    def sanity_check(self):
        if self.votes_aggregator not in RandomForest.votes_aggregators:
            ...
            raise WrongValueException(message.format(self.votes_aggregator, RandomForest.votes_aggregators))
    ...
    def build(self, data: Dataset):
        for t in range(self.nb_trees):
            ...
            tree_data = data.bootstrap_replicate(self.ensemble_random.next_bootstrap_seed(),
    ...
    def compute_ranking(self, ranking_type):
        ...
        for i, tree in enumerate(self.trees):
            attribute_scores, aggregate_scores = feature_ranking.compute_tree_contribution(tree)
            feature_ranking.update_attributes(attribute_scores, aggregate_scores, i)
    ...
    def predict(self, d: Datum, nb_trees=None):
        ...
        ensemble_stats = statistics_class.construct_from_parent(predictions_stats[0])
        ...
        if statistics_class == NodeStatisticsClassification:
            if self.votes_aggregator == RandomForest.zero_one_aggregator:
                ...
                raise WrongValueException("Wrong vote aggregator: {}".format(self.votes_aggregator))
            class_names = ensemble_stats.get_class_names()
            return class_names[arg_max(values)]
        elif statistics_class in [NodeStatisticsRegression, NodeStatisticsMultitargetRegression]:
