...
class HeuristicGini(Heuristic):
    def compute_variability(self, tree_node_stat: NodeStatisticsClassification):
...
class HeuristicVariance(Heuristic):
    def compute_variability(self, tree_node_stat: NodeStatisticsRegression):
        ...
        :return:
        """
        n = tree_node_stat.get_total_number_examples()  # n > 0 when this is called
...
class HeuristicMultitargetVariance(Heuristic):
    def compute_variability(self, tree_node_stat: NodeStatisticsMultitargetRegression):
        """
        Computes the sum of estimates of the variance E[Y^2] - E[Y]^2, for each target variable.
        Basically, the sum of HeuristicVariance.comupte_variability ...
        :param tree_node_stat:
        :return:
        """
        n = tree_node_stat.get_total_number_examples()  # n > 0 when this is called
