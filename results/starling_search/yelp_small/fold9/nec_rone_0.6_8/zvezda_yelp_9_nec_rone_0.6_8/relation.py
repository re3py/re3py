...
class Relation:
    ...
    relation_type_constant = [constant_nominal, constant_numeric, constant_multi_target]
    ...
    def __init__(self,
                 name: str,
                 related_objects: Union[Set[Tuple[str]], None],
                 file: Union[str, None],
                 types):
        self.name = name
        self.all_tuples = set()
        ...
        if p1:
            with open(file) as f:
                for whole_line in f:
                    l = whole_line[:whole_line.find("//")]  # comments
                    if l:
                        self.try_add_tuple(l)
        else:
    ...
    def try_add_tuple(self, l: str):
        """
        Converts 'r(x,y)' to (x, y) and adds it to all tuples.

        :param l: a string of form <relation name>(obj1, obj2, ...)
        """
        related_list = parse_relation_arguments(l, self.name)
        relation_tuple = tuple(Relation.intelligent_parse(v_type, v_value)
    ...
    def try_add_one_to_tuples_by_subsets_old(self, relation_tuple):
        ...
        for subset_code in subset_codes:
            ...
            key_part = []
            value_part = []
            for tuple_component, code_component in zip(relation_tuple, subset_code):
                if code_component == "1":
    ...
    def get_all_old(self, variables: List[Variable], known_values: List[int]) -> List[Tuple[Variable]]:
        """
        ...
        if not self.should_use_tuples_by_subsets():
            ...
            if len(known_values) == self.arity:
                ...
                if key_part in subset_dict:
                    ...
                    value_parts = set()
                return [merge(key_part, value_part) for value_part in value_parts]

    def get_all(self, variables: List[Variable], known_values: List[int]) -> List[Tuple[Variable]]:
        """
        :param variables: e.g., [X0(2.1), X1(?), X2('b')]
        :param known_values: list of indices of the variables that have known value, e.g., [0, 2]
        :return: list of tuples that satisfy the constraints, e.g., all triplets (x, y, z), for which
    ...
    def get_nb_all_values(self, position):
        if self.different_values[position] < 0:
            self.different_values[position] = len(self.get_all_values(position))
    ...
    def intelligent_parse(variable_type, variable_value):
        if Relation.is_numeric_type(variable_type):
            ...
            type_of_targets = Relation.get_inner_type_of_multi_target(variable_type)
            if not (variable_value.startswith('[') and variable_value.endswith(']')):
                raise WrongValueException("Multi-target value {} wrongly specified!".format(variable_value))
            target_values_string = intelligent_split(variable_value[1:-1])
            target_values = [Relation.intelligent_parse(type_of_targets, v) for v in target_values_string]
    ...
    def get_inner_type_of_multi_target(variable_type):
        ...
        if min(i0, i1) < 0:
            raise WrongValueException("Multi-target type {} wrongly specified!".format(variable_type))
    ...
    def check_has_ok_types(relation_types):
        for t in relation_types:
            if t[0] == t[0].lower() and not Relation.is_constant_type(t):
                ...
                raise WrongValueException(message.format(t, Relation.relation_type_constant))


def parse_relation_arguments(l: str, relation_name: str):
    assert l.startswith(relation_name)
    tuple_pattern = Relation.tuple_pattern.format(relation_name)
    try:
        related_str = re.search(tuple_pattern, l).group(1).strip()
    except AttributeError:
        print("Relation {}: tuple {} does not match pattern {}".format(relation_name, l, tuple_pattern))
        raise
    ...
    for o in related_list:
        if re.match(object_name, o) is None:
            if o == "":
                print("Empty string detected in {}".format(l))
            else:
                raise WrongValueException("{} in {} contains forbidden characters or is empty.".format(o, related_str))
    return related_list


def intelligent_split(l):
    inside_list = False
    ...
    for i, c in enumerate(l):
        ...
        if c == '[':
            ...
            elements.append(l[start:i])
            start = i + 1
    elements.append(l[start:])
...
def parse_relation(l):
    relation_name = parse_relation_name(l)
    related_list = parse_relation_arguments(l, relation_name)
    return relation_name, related_list


# print(intelligent_split('12, 231, das das 12, [21, 1, 21, 4 23], "nonstructural protein 1 [Influenza A virus (A/WSN/1933(H1N1))]"'))
