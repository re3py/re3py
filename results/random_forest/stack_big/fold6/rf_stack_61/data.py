...
class Datum:
    ...
    def __repr__(self):
        return "Datum({}, {}, {})".format(self.descriptive_part, self.target_part, self.weight)
...
class Dataset:
    def __init__(self,
                 s_file=None, data_file=None, target_file=None,
                 settings=None, descriptive_relations=None, target_data=None,
                 statistics=None, nb_target_instances=float('inf'), target_type=None):
        ...
        self.data_file = os.path.abspath(data_file) if data_file is not None else None
        self.target_file = os.path.abspath(target_file) if target_file is not None else None


        # settings i.e., meta data
        if s_file is not None:
            self.settings = Settings(s_file)
            self.descriptive_relations = {r.get_name(): r for r in self.settings.get_relations()}
        # target type
        ...
        # data
        all_relations_empty = not any(r.all_tuples for r in self.descriptive_relations.values())
        ...
        if target_file is not None and all_relations_empty:
            self.target_data = []
            self.read_target_from_file(target_file, self.get_target_relation().get_name())
        # statistics
        ...
        with open(file) as f:
            for l in f:
                ...
                if i >= 0:
                    ...
                    line = l.strip()
                if line:
                    if "attributes(KPoTixdjoJxSqRSEApSAGg," in line:
                        _ = 21
        ...
        with open(file) as f:
            for l in f:
                ...
                if line:
                    ...
                    target_type = self.descriptive_relations[target_relation_name].types[-1]
                    example_target = parse_relation_arguments(line, r_name)
                    example, target = example_target[:-1], example_target[-1]
                    self.add_example(Datum(tuple(example), Relation.intelligent_parse(target_type, target), 1, added))
                    added += 1
    ...
    def bootstrap_replicate(self, random_seed=25061991, per_class=False):
        ...
        if per_class and (c1 or c2):
            ...
            for i, d in enumerate(self.get_target_data()):
                ...
                if t not in classes:
                    classes[t] = []
                classes[t].append(i)
            classes = [classes[c] for c in sorted(classes)]  # always the same order ...
        else:
        ...
        for class_indices in classes:
            for chosen in Dataset._bootstrap_replicate_one_class(class_indices, r):
                successes[chosen] += 1
        new_target_data = []
        for i, nb_successes in enumerate(successes):
            if nb_successes > 0:
                datum = self.target_data[i]
                new_datum = Datum(datum.get_descriptive(),
                                  datum.target_part,
    ...
    def compute_statistics_for_data(self):
        if Relation.is_nominal_type(self.target_type):
            ...
            inner_type = Relation.get_inner_type_of_multi_target(self.target_type)
...
class NodeStatistics:
    def __init__(self, total_nb_examples=0, branch_frequencies=None, variability=None):
    ...
    def add_other_for_ensemble_prediction(self, other: 'NodeStatistics', other_weight):
        raise NotImplementedError("This should be implemented by a subclass.")
...
class NodeStatisticsClassification(NodeStatistics):
    ...
    def __str__(self):
        prediction_str = "" if self.prediction is None else "return {}".format(self.prediction)
        return "{} ({}: {})".format(prediction_str, self.class_names, self.nb_examples_per_class)
    ...
    def add_other_for_ensemble_prediction(self, other: 'NodeStatisticsClassification', other_weight):
        assert self.class_names == other.class_names
        # update for proportion voting
        for i in range(len(self.class_names)):
            self.per_class_probabilities[i] += other.per_class_probabilities[i] * other_weight
        # update for majority voting
        self.nb_examples_per_class[self.class_to_index[other.prediction]] += other_weight
    ...
    def update_per_class_probabilities(self):
        if self.total_nb_examples == 0:
            ...
            self.per_class_probabilities = [c / self.total_nb_examples for c in self.nb_examples_per_class]
    ...
    def add_example_during_split_eval(self, datum: Datum):
        ...
        self.update_with_delta_weight(self.class_to_index[datum.get_target()], datum.get_weight())
...
class NodeStatisticsRegression(NodeStatistics):
    ...
    def __str__(self):
        prediction_str = "" if self.prediction is None else "return {}".format(self.prediction)
        return "{} ({} examples)".format(prediction_str, self.total_nb_examples)
    ...
    def add_examples(self, data: List[Datum]):
        for datum in data:
            self.add_example_during_split_eval(datum)  # the same update happens

    def add_other_for_ensemble_prediction(self, other: 'NodeStatisticsRegression', other_weight):
...
class NodeStatisticsMultitargetRegression(NodeStatisticsRegression):
    ...
    def __init__(self, nb_targets, **node_statistic_args):
        ...
        self.sum1 = np.zeros(nb_targets)  # sum of w_i y_i over the examples Datum(x_i, y_i, w_i), for each target
        self.sum2 = np.zeros(nb_targets)  # sum of w_i y_i^2 over the same examples, for each target

    def __str__(self):
        prediction_str = "" if self.prediction is None else "return {}".format(list(self.prediction))
        return "{} ({} examples)".format(prediction_str, self.total_nb_examples)
    ...
    @staticmethod
    def construct_from_parent(parent_stats: 'NodeStatisticsMultitargetRegression'):
        return NodeStatisticsMultitargetRegression(parent_stats.nb_targets)

    def add_other_for_ensemble_prediction(self, other: 'NodeStatisticsMultitargetRegression', other_weight):
        self.sum1 += other.sum1
        self.sum2 += other.sum2
        self.total_nb_examples += other_weight
...
class NodeStatisticsClassificationBoosting(NodeStatisticsRegression):
    ...
    def add_other_for_ensemble_prediction(self, other: 'NodeStatisticsBinaryClassificationBoosting', other_weight):
        super().add_other_for_ensemble_prediction(other, other_weight)
        self.sum_abs1 += other_weight * other.sum_abs1


# noinspection PyAbstractClass
class NodeStatisticsBinaryClassificationBoosting(NodeStatisticsClassificationBoosting):
    @staticmethod
    def construct_from_parent(parent_stats: 'NodeStatisticsBinaryClassificationBoosting'):
...
# noinspection PyAbstractClass
class NodeStatisticsMulticlassClassificationBoosting(NodeStatisticsClassificationBoosting):
    def __init__(self, nb_classes, **node_statistic_args):
        self.nb_classes = nb_classes
        super().__init__(**node_statistic_args)

    @staticmethod
    def construct_from_parent(parent_stats: 'NodeStatisticsMulticlassClassificationBoosting'):
        return NodeStatisticsMulticlassClassificationBoosting(parent_stats.nb_classes)
...
class NodeStatisticsRegressionBoosting(NodeStatisticsRegression):
    ...
    @staticmethod
    def construct_from_parent(parent_stats: 'NodeStatisticsRegressionBoosting'):
        return NodeStatisticsRegressionBoosting()


# noinspection PyAbstractClass
class NodeStatisticsMultitargetRegressionBoosting(NodeStatisticsMultitargetRegression):
    def __init__(self, nb_targets, **node_statistic_args):
    ...
    @staticmethod
    def construct_from_parent(parent_stats: 'NodeStatisticsMultitargetRegressionBoosting'):
        return NodeStatisticsMultitargetRegressionBoosting(parent_stats.nb_targets)
