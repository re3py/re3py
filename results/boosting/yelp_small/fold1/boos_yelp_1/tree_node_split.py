...
class BinarySplit:
    ...
    def __init__(self,
                 atom_tests: List[Tuple[Relation, List[str], Aggregator]],
                 comparator: Union[Comparator, None],
                 threshold: Union[float, str, Set[str]],
                 ignore_critical_values,
                 is_variable_free):
    ...
    def __str__(self, var_dict=None):
        ...
        return "{} {} {}".format(", ".join(tests_str), self.comparator, th_for_str)
    ...
    def evaluate_all(self,
                     example: Dict[str, Variable],
                     chain_relations: List[Tuple[Relation, List[str]]],
                     chains_aggregators: List[Tuple[Aggregator]],
                     comparators: List[Comparator],
                     thresholds: List[Union[str, float]]):
        values = self.get_test_values(example, chain_relations, chains_aggregators, None, [], None, -1, None, None)
        return [c.compare(v, t) for c, v, t in zip(comparators, values, thresholds)]
    ...
    def evaluate(self, example: Dict[str, Variable]):
        ...
        outcome = self.evaluate_all(example, chain_relations, chains_agg, [self.comparator], [compare_with])
        return outcome[0]
    ...
    def get_test_values(self,
                        ...
                        chain_relations: List[Tuple[Relation, List[str]]],
                        chains_aggregators: List[Tuple[Aggregator]],
                        relation_key, aggregator_keys, tuple_id,
        ...
        if filtered_chains_aggregators:
            values_partial_all = self.get_test_value_helper(example,
                                                            chain_relations,
                                                            filtered_chains_aggregators,
            ...
            values_partial_all = []
        where_to = 0
        for agg_key, chain_aggregators, values_partial in zip(filtered_aggregator_keys,
                                                              filtered_chains_aggregators,
                                                              values_partial_all):
