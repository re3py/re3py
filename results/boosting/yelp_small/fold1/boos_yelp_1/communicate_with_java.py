from data import Dataset, Datum
...
def send_data(data: Dataset, client, wrapper):
    def l_convert(l0):
        return l.convert(l0, client)

    def m_convert(m0):
        return m.convert(m0, client)
    ...
    # relations
    relations = {r.name: l_convert(r.get_types()) for r in data.get_descriptive_data().values()}
...
def compute_test_values(target_data: List[Datum],
                        ...
                        r_key: Tuple[Tuple[str, Tuple[bool], Tuple[Union[str, int, Variable]]]],
                        a_keys: List[Tuple[str]],
                        nb_fresh_vars: int,
                        fresh_indices: List[int],
                        known_unknown: List[List[List[int]]],
                        client, wrapper):
    def l_convert(l0):
        return l.convert(l0, client)
    ...
    for aggregators in filtered_agg_chains:
        filtered_agg_chains_converted.append(l_convert([a.get_name() for a in aggregators]))
    ...
    if r_key is None:
        ...
        r_key_converted = []
        for name, bools, variables in r_key:
            n = len(bools)
            assert n == len(variables)
            new_element = ["" for _ in range(1 + 2 * n)]
            new_element[0] = name
            for i in range(n):
                new_element[1 + i] = str(bools[i]).lower()
    ...
    for known, unknown in known_unknown:
        known_unknown_converted.append(l_convert([l_convert(known), l_convert(unknown)]))
    known_unknown_converted = l_convert(known_unknown_converted)

    values = wrapper.compute_test_values(target_data_converted,
                                         target_relation_vars_converted, rc_modified_converted,
                                         filtered_agg_chains_converted,
                                         r_key_converted, a_keys_converted, nb_fresh_vars, fresh_indices_converted,
                                         known_unknown_converted)
