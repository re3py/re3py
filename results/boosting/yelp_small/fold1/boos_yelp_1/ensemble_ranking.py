...
# import re
from my_utils import float_as_string
...
class EnsembleRanking:
    ...
    def __init__(self, attributes, aggregates, ranking_type, iterations):
        ...
        if ranking_type not in EnsembleRanking.ranking_types:
            raise WrongValueException("Your ranking type: {}. Allowed-ones: {}".format(ranking_type,
                                                                                       EnsembleRanking.ranking_types))
        self.ranking_type = ranking_type

    def __str__(self):
        names = ["Attributes", "Attributes summed", "Aggregators"]
        dictionaries = [self.attributes, self.attributes_summed, self.aggregates]
        sorted_keys = [EnsembleRanking.sorted_keys_dictionary(d) for d in dictionaries]
        max_len = max([0] + [len(key) for keys_sums in sorted_keys for key in keys_sums[0]])
        lines = []
        line_pattern = "{{: <{}}}: {{}}; iterations: [{{}}]".format(max_len)
        for name, (keys, importance_sums), dictionary in zip(names, sorted_keys, dictionaries):
            lines.append(name)
            for k in keys:
                vs = dictionary[k]
                formatted_sum = float_as_string(importance_sums[k], EnsembleRanking.nb_places)
                formatted_vs = ", ".join([float_as_string(importance, EnsembleRanking.nb_places) for importance in vs])
                lines.append(line_pattern.format(k, formatted_sum, formatted_vs))
            lines.append("")
        return "\n".join(lines)

    def update_attributes(self,
                          attributes_contributions,
                          aggregates_contributions,
                          iteration):
        for d_one, d_all in zip([attributes_contributions, aggregates_contributions],
        ...
        for relation_name_arguments, v in attributes_contributions.items():
            relation_name = relation_name_arguments[:relation_name_arguments.find("[")]
    ...
    def compute_tree_contribution(self, tree: DecisionTree):
        ...
        for node in tree:
            if not node.is_leaf():
                attributes, aggregates = EnsembleRanking.get_attributes_and_aggregates(node)
                for l, d in zip([attributes, aggregates], [attributes_scores, aggregates_scores]):
                    for a in l:
                        if a not in d:
                            d[a] = 0.0
                if self.ranking_type == EnsembleRanking.genie3:
                    importance = EnsembleRanking.compute_genie3_importance(node)
                elif self.ranking_type == EnsembleRanking.symbolic:
                    nb_examples = tree.root_node.get_stats().get_total_number_examples()
                    importance = EnsembleRanking.compute_symbolic_importance(node, nb_examples)
                elif self.ranking_type == EnsembleRanking.prob_dist:
                    importance = EnsembleRanking.compute_weighted_probability_distance_importance(node)
                else:
                    raise NotImplementedError("Ranking for type {} not implemented.".format(self.ranking_type))
    ...
    def compute_genie3_importance(node: TreeNode):
        ...
        var_children = [c.get_stats().get_variability() for c in node.get_children()]
        return var_here - sum(p * v for p, v in zip(node.get_stats().get_branch_frequencies(),
                                                    var_children))
    ...
    def sorted_keys_dictionary(dictionary):
        ...
        return sorted(dictionary.keys(), key=lambda k: sums[k], reverse=True), sums
